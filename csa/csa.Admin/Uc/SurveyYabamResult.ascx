<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SurveyYabamResult.ascx.cs" Inherits="csa.Member.Uc.SurveyYabam" %>
...
<script>
    ...
    function loadYabam() {
        ...
        vmSurvey = JSON.parse($('#<%= hfModelView.ClientID %>').val())
        var ans = JSON.parse(vmSurvey.Answer)

        vmSurvey.States.splice(0, 0, { StateId: 0, Name: 'Select an option', CountryId: 0 })
        vmSurvey.States.forEach(function (item) {
            $('[name="a1"]').append(`<option value='${item.StateId}'>${item.Name}</option>`)
        })

        vmSurvey.Sectors.splice(0, 0, { Key: 0, Text: 'Select an option' })
        vmSurvey.Sectors.forEach(function (item) {
        ...
        $('[name="a2"]').on('change', function () {
            ...
            vmSurvey.JobPositions.filter(x => x.RefValue == $(this).val()).forEach(function (item) {
