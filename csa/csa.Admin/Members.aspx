<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Members.aspx.cs" Inherits="csa.Admin.Members" %>
                ...
                <div class="col-12">
                    <div class="d-flex align-items-lg-center flex-lg-row flex-column">
                        <div class="flex-grow-1">
                            <h4 class="fs-16 mb-1">Member List</h4>
                            ...
                            <div class="col-md-3 col-sm-12 d-flex align-items-center"><h5 class="m-0 p-0 "></h5></div>
                                <div class="col-md-9 col-sm-12 ">
                                    <div class="row justify-content-end gap-3">
                                        ...
                                        <button class="btn btn-primary col-md-2 col-sm-12" id="btnExportSurvey" type="button">Export Survey</button>    
                                        <button class="btn btn-primary col-md-3 col-sm-12" id="btnExportFinance" type="button">Export Finance</button>    
                                        <button class="btn btn-primary col-md-3 col-sm-12" id="btnExportDemographicMarketAnalysis" type="button">Export Market</button>    
                                        <button class="btn btn-primary col-md-3 col-sm-12" id="btnExportRM" type="button">Export RM</button>    
                                        <button class="btn btn-primary col-md-3 col-sm-12" id="btnExportOperation" type="button">Export Operation</button>    
                                        <button class="btn btn-primary col-md-3 col-sm-12" id="btnExportCredit" type="button">Export Credit</button>    
    ...
    <script type="text/javascript">
        ...
        function initMemberGV() {
            gvData = $("#gvData").DataTable({
                ...
                //'scrollCollapse': true,
                'ajax': {
                    'url': '<%=Page.ResolveUrl("~/Member/GetMemberGV")%>',
                    'type': "POST",
                    'data': function (e) {
                        let search = '';
        ...
        $('#btnExportSurvey').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportSurvey?startDate=&endDate=`).then(response => {
        ...
        $('#btnExportFinance').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportFinance?startDate=&endDate=`).then(response => {
                // Create a URL for the Blob object
                const url = window.URL.createObjectURL(new Blob([response.data]));
        ...
        $('#btnExportDemographicMarketAnalysis').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportDemographicMarketAnalysis?startDate=&endDate=`).then(response => {
                // Create a URL for the Blob object
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const contentDisposition = response.headers.get('Content-Disposition');
                // If Content-Disposition is present, extract the filename
        ...
        $('#btnExportRM').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportRM?startDate=&endDate=`).then(response => {
                // Create a URL for the Blob object
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const contentDisposition = response.headers.get('Content-Disposition');
                // If Content-Disposition is present, extract the filename
        ...
        $('#btnExportOperation').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportOperation?startDate=&endDate=`).then(response => {
                // Create a URL for the Blob object
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const contentDisposition = response.headers.get('Content-Disposition');
                // If Content-Disposition is present, extract the filename
        ...
        $('#btnExportCredit').click(async function (e) {
            ApiHelper.download(window.location.origin + `/member/ExportCredit`).then(response => {
                // Create a URL for the Blob object
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const contentDisposition = response.headers.get('Content-Disposition');
                // If Content-Disposition is present, extract the filename
