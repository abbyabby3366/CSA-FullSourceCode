<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="AudienceTag.aspx.cs" Inherits="csa.Admin.AudienceTag" %>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="col-md-3 col-sm-12 d-flex align-items-center"><h5 class="m-0 p-0 "><%--Audience Tag List--%></h5></div>
                                <div class="col-md-9 col-sm-12 ">
                                    <div class="row justify-content-end gy-3">
                                        ...
                                        <a href="<%=Page.ResolveUrl("~/AddNewAudienceTag") %>" class="btn btn-soft-primary waves-effect waves-light col-md-3 col-sm-12">Add New Audience Tag</a>
                                    </div>
                                </div>
                            </div>
                         </div>

                            <table id="gvAudienceTag" class="table dt-responsive nowrap align-middle" style="width: 100%">
...
<asp:Content ID="Script" ContentPlaceHolderID="ScriptContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function initAudienceTagGV() {
            gvTable = $("#gvAudienceTag").DataTable({
                'responsive': false,
                //'buttons': [
                //    'print',
                //    'copyHtml5',
                ...
                //'scrollCollapse': true,
                'ajax': {
                    'url': '<%=Page.ResolveUrl("~/Tag/GetTagGV")%>',
                    'type': "POST",
                    'data': function (e) {
                        let search = '';
                ...
                'columnDefs': [
                    {
                        ...
                        'render': function (d, t, r, m) {
                            return `<div class="hstack gap-3 fs-13 justify-content-center">
                                               <a href="<%=Page.ResolveUrl("~/AudienceTagDetails")%>?id=${d}" class="text-success">Edit</a>
                                               <a href='#' class="text-danger" onclick='onDeleteTag(${d})'>Delete</a>
                                           </div>`
                        }                        
                    },
                    {
        ...
        function onDeleteTag(tagId) {
            async function doDelete() {
                ...
                try {
                    const res = await ApiHelper.post(window.location.origin + '/Tag/DeleteTag', payload)
                    if (!res.data.Error) {
                        gvTable.ajax.reload();
                    } else {
